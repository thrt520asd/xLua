...
{{
    %s
                    ...
                    struct {Signature}
                    {{");
                    for (int i = 0; i < FieldSignatures.Count; i++)
                    {
                        string line = "";
                        if(Signature.StartsWith("N_") && i == NullableHasValuePosition){
                            line = FileExporter.SToCPPType(FieldSignatures[i]) + " hasValue;";
                        }else{
                            line = FileExporter.SToCPPType(FieldSignatures[i]) + $" p{i};";
                        }
                        sb.AppendLine(line);
                    }
                    sb.AppendLine("};");
